...
`activity supplies <id>` 和 `inspect/trace/home/reset <id>` 诊断。测试问询时
...
- `adb forward tcp:3000 tcp:3000` 后运行 `tools/ai_admin_smoke.py`，会通过本地回环协议创建或
  登录一次性管理员，执行 `status`、`validate` 和五角色 `selftest`。增加 `--scenarios` 后还会
  顺序运行五个隔离非致死战斗场景，并触发苏晚棠带测试银两的真实补给活动。
