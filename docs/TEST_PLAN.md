...
- `tools/plot_arc_hardening.py` 默认执行全剧情静态加固：53 个剧情源文件的 UTF-8/GB18030
  无损往返、runtime 压缩包逐文件一致性、ZJ 控制串与按钮长度、长文本、实例关闭契约、
  世界正式名/真实朝代、师门写操作和转世脱钩扫描；`--runtime` 使用专用 QA 账号验证历史
  存档惰性初始化、schema v1 迁移、损坏阶段安全重置、完成状态归一、未来 schema 原子拒绝，
  并解析 `plotadmin audit` 的状态体积和实例引用。
- `tools/test_plot_platform.py` 额外锁定 schema v2、合法阶段注册表、未来版本拒绝和管理员审计
  输出；五章 smoke/recovery 脚本继续作为完整身份/选择/解法/强停矩阵入口。
...
2026-07-21 全篇加固与固定武学背景回归结果：

- `tools/plot_arc_hardening.py` 静态检查通过（54 个剧情文件）；runtime 逐文件 GB18030 内容与
  源码一致，runtime 条目 12,159，runtime SHA-256 为
  `9d07ac3e7da385d2ccbc5bb40d8578f366387e234b2d0dffcc84d02afb46ca3b`；
- 已安装 APK 覆盖升级通过：旧 runtime `5aebf990587d4be3b313a992b38a1bed7f1e0f2440e5ec933bde480380141589`，
  当前 APK SHA-256 为 `110ed315e18ce3327659b2e686d872b25a46de83cc3639c91998877ce82a964a`，
  QA 角色存档 SHA-256 升级前后保持一致；
- 五章主流程矩阵通过：第一章 9 组、第二章 9 组、第三章 9 组、第四章 9 组、第五章 12 组；
  第二至第五章强停恢复全部通过，覆盖 NPC 受伤/死亡、证据半成品、Boss 阶段和完成幂等；
- 六项重点 profile 的固定武学背景、技能映射、准备、兵器、核心绝招前置和真实 `perform` 均通过 runtime
  验证；闻守拙在三档 Boss 阶段依次成功执行 `staff.chan`、`staff.ban`、`staff.wugou`，花鹞子
  成功执行持长鞭的 `whip.chan`；
- 真实 runtime 自检通过：`PLOT_SELFTEST ok=1 schema=2`、`PLOT_COMBAT_SELFTEST ok=1 profiles=6`；
  APK 重装后的 `plotadmin audit` 为 `state_chars=6133 instance_refs=0 objects=570`；
- 本轮强度验收只检查经验主导的属性/武学/兵器配置、奇经八脉加成和 Boss 阶段，不以战斗时长作锚点。
