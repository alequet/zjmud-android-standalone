...
- 已生成可侧载的 ARM64 Debug APK，当前约 29.2 MiB。
...
- runtime 包含 22,863 个文件，当前 SHA-256 为
...
| zjmud 上游源码归档 | [GitHub ZIP](https://github.com/MudRen/zjmud/archive/c56a166380d74858d7b4f0ba2817478ccea6b83d.zip) |
| zjmud 上游 ZIP SHA-256 | `61ce705fd694bcc3ba4619c4a475e020fe4237df226fb827697de0d682e8b014` |
...
公开仓库中的 Web 前端不能直接参与本项目构建。它存在 ANSI 默认状态、聊天区高度、建角
性别控件以及本地登录/注册流程问题；本地已修复版本与公开上游的完整差异保存在独立的
[`tools/zjmud_patch/web_frontend.patch`](tools/zjmud_patch/web_frontend.patch) 中，并随本仓库
版本控制。`tools/import_zjmud.sh` 校验公开 ZIP 后，必须先应用并校验该 `zjmud_patch`，再执行
Android 单机化的 mudlib 和 WebView 补丁。这样公开上游、通用 zjmud 修复和 Android 专用改动
保持为三个可审计层次。

导入脚本会拒绝哈希不一致的上游 ZIP 或 `zjmud_patch`，避免字节级补丁静默应用到未知版本。
仓库跟踪确定性生成的 runtime 资产，但不跟踪上游 ZIP、Windows 二进制和构建缓存。
...
环境版本见 [开发环境](docs/DEVELOPMENT_ENVIRONMENT.md)。重新导入 zjmud 资源时，先下载固定
公开提交的 GitHub ZIP；导入脚本会自动应用仓库中的 `zjmud_patch`：

```bash
curl -fL -o ~/zjmud-c56a166.zip \
  https://github.com/MudRen/zjmud/archive/c56a166380d74858d7b4f0ba2817478ccea6b83d.zip
./tools/import_zjmud.sh ~/zjmud-c56a166.zip
