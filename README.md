...
- 已生成可侧载的 ARM64 Debug APK，当前约 21.8 MiB。
...
| `ai_players.patch` / `ai-playerd.c` | 常驻加载 5 个持久化 AI 玩家；它们使用真实玩家对象按角色分时日程巡游、驻足、补给和交谈。结构化感知玩家进出、说话、问询、附近/自身战斗与时段变化；关系、地点和时段共同改变招呼与对白。v1.6 增加多点巡游、安全休息、真实购物及受控同区域结伴会面，并保留带快照恢复的隔离非致死战斗、购买/消费后置校验、适配器失败指标、账号隔离、有界寻路和动作白名单。训练、通用组队与任意命令仍保持显式不支持。 |
