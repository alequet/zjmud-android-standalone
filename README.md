...
- 已生成可侧载的 ARM64 Debug APK，当前约 21.9 MiB，SHA-256 为
  `34734bea4d84771b105ff5add5157114d728df6203f4e79b4ee8fdc4caacf422`。
...
- runtime 归档包含 12,159 个条目，当前 SHA-256 为
  `ddecd81701d120bd8f5bf41e8e23ea741bb3f61f2048fab420915922135d8fb6`。
- 5 名 AI 玩家已完成 30 分钟稳定性回归：398 次累计动作、无新增错误或动作失败，
  对象数经过回收后净减少 245，RSS 净增约 1.28 MiB，5 份角色档均周期更新。
- 当前仍是个人单机版工程，不是可替代原联网服的通用发行版。

## v1.1 更新内容（2026-07-21）

### 主线剧情当前状态

- “乡书篇”前五章已经完成正式接入：第一章《一纸乡书》、第二章《旧印新痕》、第三章《夜渡无声》、
  第四章《故园来客》和第五章《百川归处》均登记为 `implemented=1`，按前置完成顺序开放。
- 主线从故乡来信和义仓簿开始，逐步追查“回浪印”、百川会和无面局：玩家会经历三方取证、旧仓辨印、
  夜船救援、乡书辨伪和旧盐仓高潮，最终识破闻守拙只是有罪的掌簿前台，沈观澜仍作为后续篇章的幕后线索。
- 五章共用 `hometown_letters_arc_01` 篇章状态；分支主要保留关系、证据、入口便利和人物回响，不复制后续主线。
  第五章以 `archive_disposition` 二选一收束“留全簿追责”或“隐名存证”，两种尾声奖励等价并回到扬州开放世界。
- 剧情平台已固定 schema v2、个人实例、幂等结算、关键 NPC 受伤/非致死 Boss 生命周期、强停恢复和 AI 缺席时的替代路径。
  五章 smoke/recovery 矩阵与 runtime 自检已通过；第六章以后暂未实现。

### 如何开始原创剧情

- 角色必须先完成投胎并正常进入世界；剧情不要求拜师，也不会修改师门、出身或转世状态。
- 新角色第一次抵达安全客栈或扬州时，会收到杜宽托人送来的“同乡旧信”提示。老角色可直接前往扬州武庙，
  选择“乡书旧闻”，或到扬州驿站向杜宽询问“乡书”。
- 游戏内输入 `plot` 可打开剧情总览，输入 `plot origin_letter_01` 查看第一章《一纸乡书》的日志和入口。
  在扬州驿站确认接信后，按页面按钮或使用 `plot act origin_letter_01 accept` 开始调查。
- 首章需要至少听取会馆、衙门、粮行三方中的两方证词，再追踪扬州北门外的废仓；裴九可通过战斗、说服或赎回处理，
  后续谜题和账簿去向都从剧情日志继续推进。战斗不是唯一解法。
- 完成当前章节后，下一章会在 `plot` 总览中按前置顺序变为“可接取”。可用
  `plot returning_mark_02`、`plot silent_crossing_03`、`plot visitors_from_home_04` 和
  `plot where_rivers_end_05` 查看对应章节日志；章节按钮会负责进入个人剧情场景。
- 断线、退出或服务重启后，重新输入对应的 `plot <章节编号>` 即可从已保存阶段继续；重复查看、推进和领奖不会重复结算。

### 武学与战斗调整

- 原创剧情战斗改为经验主导的固定武学背景：奇经八脉只作同档有限加强，不以战斗时长或无限气血作为强度锚点；
  重点角色冻结门派来源、技能映射、兵器/徒手准备和阶段绝招，并逐档验证核心 `perform` 前置。
- 已落地的剧情 profile 包括：裴九的钢刀/五虎断门刀、青篷客的少林俗家徒手体系、罗七娘的水路护运刀法、
  花鹞子的云龙鞭法，以及闻守拙的丐帮旧支棍法；闻守拙按经验档开放「缠字诀」「绊字诀」「天下无狗」阶段。
- 新增商城洗点丹、性格丹（各 1 元宝）。洗点保留每项最低 13 和总点数约束，移除 39 点上限；性格切换和洗点
  只清理明确不兼容的武学，武器、基础技能、内力和经验等成长前置不会误删。左右互搏不再额外限制悟性。
- `character_skilld` 与 `attribute_skilld` 将性格兼容规则和武学属性门槛集中管理；武学扩展候选已完成评审，当前版本尚未直接导入新增候选技能。
...
| 商城重置丹 | 随身商城出售 1 元宝的洗点丹和性格丹；使用后直接弹出先天属性分配或性格选择窗口，确认成功后才消耗物品。洗点单项只保留最低 13 和总点数约束，不设 39 上限；性格切换只检查明确的性格兼容规则，洗点只检查武学的属性门槛，且左右互搏不再限制悟性。当前武器、基础技能、内力、经验等成长前置不会导致已持有武学被删除。 |
...
- [武学扩展计划](docs/MARTIAL_ARTS_EXPANSION_PLAN.md)
- [前五章剧情实施计划](docs/STORY_IMPLEMENTATION_PLAN.md)
- [原创剧情战斗规范](docs/ORIGINAL_PLOT_COMBAT_SPEC.md)
- [原创剧情 NPC 生命周期约束](docs/ORIGINAL_PLOT_NPC_LIFECYCLE.md)
