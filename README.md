...
- 已生成可侧载的 ARM64 Debug APK，当前约 21.8 MiB，SHA-256 为
  `147c5c94ff51c0be8a76982a54de35252c796fc3af625d3fadaaa1583c2e675b`。
...
- runtime 归档包含 12,083 个条目，当前 SHA-256 为
...
| `ai_players.patch` / `ai-playerd.c` | 常驻加载 5 个持久化 AI 玩家；它们使用真实玩家对象按角色分时日程巡游、驻足、补给和交谈。结构化感知玩家进出、说话、问询、附近/自身战斗与时段变化；关系、地点和时段共同改变招呼与对白。v1.6 增加多点巡游、安全休息、真实购物及受控同区域结伴会面；v1.7 为活动状态增加 schema v2、白名单迁移、非法/过期状态安全取消、结伴对账及购买/消费检查点。系统保留带快照恢复的隔离非致死战斗、适配器失败指标、账号隔离、有界寻路和动作白名单。训练、通用组队与任意命令仍保持显式不支持。 |
...
- Debug APK 额外包含显式故障注入 receiver，只能请求非导出的服务自杀，用于验证 `:mud`
  异常退出恢复；release source set 不包含该组件，服务端入口也会检查应用可调试标志。
...
- AI 玩家跨区域寻路、任务规划、经济行为、主动组队/PK、长期记忆和外部语言模型接入。
