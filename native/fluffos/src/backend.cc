...
#ifdef __ANDROID__
namespace {
int g_android_shutdown_fd = -1;
struct event *g_android_shutdown_event = nullptr;
std::atomic_bool g_android_shutdown_requested{false};

void save_android_users() {
  int saved = 0;
  for (auto *user : users()) {
    if (user == nullptr || user->ob == nullptr) continue;
    (void)safe_apply("save", user->ob, 0, ORIGIN_DRIVER);
    saved++;
  }
  debug_message("Android autosave processed %d interactive user(s).\n", saved);
}

void schedule_android_autosave() {
  save_android_users();
  add_gametick_event(std::chrono::seconds(30),
                     tick_event::callback_type(schedule_android_autosave));
}

void on_android_shutdown(int, short, void *) {
  uint64_t value;
  while (read(g_android_shutdown_fd, &value, sizeof(value)) == sizeof(value)) {
  }
  debug_message("Android graceful shutdown started.\n");
  save_android_users();
  sync();
...
void request_android_shutdown() {
  g_android_shutdown_requested.store(true);
  if (g_android_shutdown_fd == -1) return;
  const uint64_t value = 1;
  (void)write(g_android_shutdown_fd, &value, sizeof(value));
}
#endif
...
#ifdef __ANDROID__
  g_android_shutdown_fd = eventfd(0, EFD_CLOEXEC | EFD_NONBLOCK);
  if (g_android_shutdown_fd == -1) {
    fatal("Unable to create Android shutdown eventfd.\n");
  }
  g_android_shutdown_event =
      event_new(g_event_base, g_android_shutdown_fd, EV_READ | EV_PERSIST, on_android_shutdown, nullptr);
  if (g_android_shutdown_event == nullptr || event_add(g_android_shutdown_event, nullptr) == -1) {
    fatal("Unable to register Android shutdown event.\n");
  }
  if (g_android_shutdown_requested.load()) request_android_shutdown();
#endif
