...
#ifdef __ANDROID__
extern "C" char *__crypt_des(const char *key, const char *setting, char *output);
extern "C" char *__crypt_md5(const char *key, const char *setting, char *output);
#endif
...
#ifdef __ANDROID__
static char *android_des_crypt(const char *key, const char *setting) {
  static char output[21];
  return __crypt_des(key, setting, output);
}

static char *android_md5_crypt(const char *key, const char *setting) {
  static char output[64];
  return __crypt_md5(key, setting, output);
}

static bool is_des_crypt_setting(const char *setting) {
  const char *alphabet = "./0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz";
  size_t length = strlen(setting);
  return (length == 2 || length == 13) && strchr(alphabet, setting[0]) &&
         strchr(alphabet, setting[1]);
}

static char *android_crypt(const char *key, const char *setting) {
  if (is_des_crypt_setting(setting)) {
    return android_des_crypt(key, setting);
  }
  if (strncmp(setting, "$1$", 3) == 0) {
    return android_md5_crypt(key, setting);
  }
  return custom_crypt(key, setting, 0);
}

#undef CRYPT
#define CRYPT(x, y) android_crypt(x, y)
#define OLDCRYPT(x, y) android_des_crypt(x, y)
#else
