...
LPC_INT new_call_out(object_t *ob, svalue_t *fun, std::chrono::milliseconds delay_msecs,
  ...
  DBG_CALLOUT("new_call_out: /%s delay msecs %lld\n", ob->obname,
              static_cast<long long>(delay_msecs.count()));
...
void call_out(pending_call_t *cop) {
  ...
  const auto current_time = cop->is_walltime
                                ? std::chrono::duration_cast<std::chrono::milliseconds>(
                                      std::chrono::high_resolution_clock::now().time_since_epoch())
                                      .count()
                                : g_current_gametick;
  DBG_CALLOUT("  target_time: %llu vs current: %llu\n",
              static_cast<unsigned long long>(cop->target_time),
